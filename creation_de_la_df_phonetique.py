...
if __name__ == "__main__":
    ...
    if file_name_txt == 'all':
        ...
        with open(path_texts + '/{}'.format(file_name_txt), encoding='UTF-8') as f:
            text_all_rappeur = f.read()
    ...
    for i in range(1, NOMBRE_CONSONNE + 1):
    ...
    for mot in set_mot_unique:
        mot_phonetique = transform_mot_to_phonetique(mot)
        tab_voyelle, tab_consonne = find_voyelle_and_consonne_in_phonetique(mot_phonetique)
        if len(tab_voyelle) > NOMBRE_VOYELLE or len(tab_consonne) > NOMBRE_CONSONNE:
            print(mot)
        else:
            dict_mot_phonetique[mot] = mot_phonetique

    list_mot_unique = sorted(list(dict_mot_phonetique.keys()))
    ...
    phonetique_df.to_pickle("./pickles_csv/phonetique_df_{}.pkl".format(file_name))
